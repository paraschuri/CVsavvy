        ...
        <nav className="w-full shadow-md shadow-[#71767c] z-10 sticky top-0 left-0 bg-[#474B4F]">
            <div className='md:flex max-w-[1300px] bg-[#474B4F] m-auto items-center justify-between py-[0.3rem] px-10 lg:px-20'>
                ...
                <ul className={`absolute left-0 w-full lg:w-auto lg:flex lg:items-center lg:gap-4 pb-6 lg:pb-0 lg:static bg-[#474B4F] lg:z-auto z-[-1] lg:pl-0 pl-10 ${isOpen ? 'top-12 transition-all duration-400 ease-in':'top-[-500px]'}`}>
                    ...
                    <li className="'lg:ml-8 lg:my-0 my-6">
                        <Link to="/companies" className={`hover:text-lightGreen ${active==="companies"?"text-lightGreen":"text-white"}`} onClick={()=>{setIsOpen(false);setActive("companies")}}>COMPANIES</Link>
                    </li>
                    <li className="'lg:ml-8 lg:my-0 my-6">
                        <Link to="/" className={`hover:text-lightGreen ${active==="contact"?"text-lightGreen":"text-white"}`} onClick={()=>{setIsOpen(false);setActive("contact")}}>CONTACT US</Link>
                    </li>
                    <Link to="/start" className="lg:hidden bgGradient py-2 mr-4 my-1 px-6 text-xs rounded-3xl font-semibold" onClick={()=>{setIsOpen(false);setActive("")}}>GET STARTED</Link>
                ...
                <div className="hidden md:flex mt-1"> 
                    <Link to="/start" className={`bgGradient ${isOpen?"hidden":""} py-2 my-1 px-6 text-xs rounded-3xl font-semibold`} onClick={()=>{setActive("")}}>GET STARTED</Link>
